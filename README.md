...
Plain is a native macOS, document-first browser for the readable web. The current release is **Plain 1.1.2**.
...
- Plain 1.1.2 is ad-hoc signed and not notarized, so macOS may show a first-open warning on downloaded builds.
...
Plain is currently **Plain 1.1.2**. It is useful for readable pages today, with one deliberate boundary: pages that require a full web-app browser should open in the default browser.
