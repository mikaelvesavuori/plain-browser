...
Plain News lets you add sources from a categorized library, add custom RSS or web sources, choose a time window, add interests, and run a local reading pass. Digest items open in Plain's reader, and the back button returns to the digest.
