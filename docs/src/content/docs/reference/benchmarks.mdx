...
For Plain `1.1.0`, the fresh May 22, 2026 benchmark run supports byte, request, speed, and 0-page-JavaScript claims, but the full marketing gate did not pass because resident-memory reductions were below the 30% threshold. Do not publish a current `1.1.0` memory-reduction claim unless a later run passes that policy.
